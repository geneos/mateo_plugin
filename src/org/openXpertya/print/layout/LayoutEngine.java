...
public class LayoutEngine implements Pageable,Printable,Doc {
    ...
    private void createStandardHeaderFooter()
	{
		...
		insertLogoAndVersion();
		//
		PrintElement element = new StringElement("@Page@ @*Page@ @of@ @*PageCount@", font, color, null, true);
		element.layout (m_header.width, 0, true, MPrintFormatItem.FIELDALIGNMENTTYPE_Center);
		element.setLocation(m_header.getLocation());
		m_headerFooter.addElement(element);
		/*
    ...
    private void insertLogoAndVersion() {
    	MPrintTableFormat tf = m_format.getTableFormat();
		Color color = tf.getPageHeaderFG_Color();
		...
		element = new StringElement("@Version@: "+m_format.getDescription(), new Font( tf.getStandard_Font().getName(),Font.BOLD,tf.getStandard_Font().getSize()), color, null, true);
		element.layout (m_header.width, 0, true, MPrintFormatItem.FIELDALIGNMENTTYPE_TrailingRight);
		Point auxPoint = m_header.getLocation();
		auxPoint.setLocation(auxPoint.getX(), auxPoint.getY()+tf.getStandard_Font().getSize()+2);
		element.setLocation(auxPoint);
		m_headerFooter.addElement(element);
