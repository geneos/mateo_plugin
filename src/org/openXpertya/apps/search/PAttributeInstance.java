...
public class PAttributeInstance extends CDialog implements ListSelectionListener {
    ...
    private String s_sqlFrom = " M_AttributeSetInstance asi INNER JOIN M_Product p ON (p.M_Product_ID = ?) JOIN M_Storage s ON (s.M_AttributeSetInstance_ID=asi.M_AttributeSetInstance_ID AND s.M_Product_ID=p.M_Product_ID) LEFT OUTER JOIN M_Locator l ON (s.M_Locator_ID=l.M_Locator_ID AND l.M_Warehouse_ID=?) ";
    ...
    private void enableButtons() {
        ...
        if( enabled ) {
            ...
            if( ID != null ) {
                ...
                m_M_Locator_ID = Integer.valueOf(((KeyNamePair)m_table.getValueAt( row,5 )).getID());
