    ...
    protected boolean afterSave( boolean newRecord,boolean success ) {
        if( success && (getC_OrderLine_ID() != 0) ) {
            ...
            if( getM_InOutLine_ID() != 0 ) {
            	...
            	//Movimiento positivo: Chequeo que el remanente no sea negativo
            	if (getQtyMovement().signum() >= 0){
            		if (orderLine.getQtyReserved().subtract(getQtyMovement()).signum() == -1)
            			orderLine.setQtyReserved(BigDecimal.ZERO);
            		else
                        orderLine.setQtyReserved( orderLine.getQtyReserved().subtract( getQtyMovement()));
            	}
            	
            	//Movimiento negativo: Chequeo que lo remanente no supere lo ordenado
            	if (getQtyMovement().signum() == -1){
            		if (orderLine.getQtyReserved().subtract(getQtyMovement()).compareTo(orderLine.getQtyOrdered()) == 1)
            			orderLine.setQtyReserved(orderLine.getQtyOrdered());
            		else
            			orderLine.setQtyReserved( orderLine.getQtyReserved().subtract( getQtyMovement()));
            	}
            ...
            if( getC_InvoiceLine_ID() != 0 ) {
				...
				if(isDebit){
	            	...
	            	//FIXME: Ver esto... no entiendo porque descuenta ordenado si se esta facturando???
