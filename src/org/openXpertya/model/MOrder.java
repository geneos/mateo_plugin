...
public class MOrder extends X_C_Order implements DocAction {
	...
	public static MOrder copyFrom(MOrder from, Timestamp dateDoc, int C_DocTypeTarget_ID, boolean isSOTrx, boolean counter, boolean copyASI, String trxName) {
		...
		// null,"Recien llegado al copyfrom ",null,
		...
		if (counter) {
			...
			// JOptionPane.showMessageDialog(
			// null,"El ad_org_id en el if es= "+a.getAD_OrgBP_ID(),null,
			// JOptionPane.INFORMATION_MESSAGE );
			// JOptionPane.showMessageDialog(
			// null,"El warehouse en el if es= "+c.getM_Warehouse_ID(),null,
			// JOptionPane.INFORMATION_MESSAGE );
		...
		// JOptionPane.showMessageDialog(
		// null,"Antes de crear laslineas la id = "+from.getC_Order_ID(),null,
		// JOptionPane.INFORMATION_MESSAGE );
	...
	public static boolean isDiscountsApplied(MOrder order) {
		...
		List<MDocumentDiscount> discounts = MDocumentDiscount.getOfOrder(order.getID(), order.getCtx(), order.get_TrxName());
		return totalDiscountsAmt.compareTo(BigDecimal.ZERO) != 0 || (discounts != null && discounts.size() > 0);
	}
	...
	public void setC_DocTypeTarget_ID(String DocSubTypeSO_x) {
		String sql = "SELECT C_DocType_ID FROM C_DocType " + "WHERE AD_Client_ID=? AND AD_Org_ID IN (0," + getAD_Org_ID() + ") AND DocSubTypeSO=? "
	...
	public void setC_DocTypeTarget_ID() {
		...
		String sql = "SELECT C_DocType_ID FROM C_DocType " + "WHERE AD_Client_ID=? AND AD_Org_ID IN (0," + getAD_Org_ID() + ") AND DocBaseType='POO' "
				+ "ORDER BY AD_Org_ID DESC, IsDefault DESC";
	...
	public int copyLinesFrom(MOrder otherOrder, boolean counter, boolean copyASI) {
		...
		// Si el otro pedido es un presupuesto y está vencido, entonces se
		// actualizan
		...
		boolean isOrderTransferred = docType.getDocTypeKey().equalsIgnoreCase(MDocType.DOCTYPE_Pedido_Transferible);
		...
		for (int i = 0; i < fromLines.length; i++) {
			...
			// JOptionPane.showMessageDialog(
			// null,"En copylinesfrom,el c_order_id="+getC_Order_ID(),null,
			// JOptionPane.INFORMATION_MESSAGE );
	...
	public String toString() {
		StringBuffer sb = new StringBuffer("MOrder[").append(getID()).append("-").append(getDocumentNo()).append(",IsSOTrx=").append(isSOTrx())
	...
	public int getC_Invoice_ID(boolean onlyDebits) {
		...
		if (onlyDebits) {
			sql.append(" AND dt.docbasetype IN ('" + MDocType.DOCBASETYPE_APInvoice + "','" + MDocType.DOCBASETYPE_ARInvoice + "') ");
		}
	 ...
	 * @return
	 */
	// Ader: nuevo beforeSave : evitar leer TODAS las lineas desde la DB y si
	// solo se requiere
	...
	protected boolean beforeSave(boolean newRecord) {
		...
		if (!newRecord && is_ValueChanged("M_Warehouse_ID")) {
			// TODO: pasar esto a una solo acceso; actualmente no molesta desde
			// el tpv
		...
		if (getM_PriceList_ID() == 0) {
			int ii = DB.getSQLValue(null, "SELECT M_PriceList_ID FROM M_PriceList " + "WHERE AD_Client_ID=? AND IsSOPriceList=? " + "ORDER BY IsDefault DESC",
		...
		MDocType docType = MDocType.get(getCtx(), getC_DocTypeTarget_ID(), get_TrxName());
		// si el registro es nuevo, no pueden haber lineas, no tiene sentido el
		// chequeo
		// si ni la lista de precios ni la moneda cambio, tampoco hay que hacer
		// el chequoe
		boolean needCheckQtyLines = !newRecord && (is_ValueChanged("M_PriceList_ID") || is_ValueChanged("C_Currency_ID"));
		if (needCheckQtyLines) // si newRecord necesariamete se tiene 0 lineas
		{
			...
			if (is_ValueChanged("M_PriceList_ID") && qtyLines > 0 && !docType.isAllowChangePriceList()) {
		...
		if (priceListCurrency != getC_Currency_ID()
				&& MCurrency.currencyConvert(new BigDecimal(1), priceListCurrency, getC_Currency_ID(), getDateOrdered(), getAD_Org_ID(), getCtx()) == null) {
		...
		// tarifa, sino error
		if (!newRecord && docType.isAllowChangePriceList() && (is_ValueChanged("M_PriceList_ID") || is_ValueChanged("DateOrdered"))
				&& (MOrder.DOCSTATUS_Drafted.equals(getDocStatus()) || MOrder.DOCSTATUS_InProgress.equals(getDocStatus()))) {
			...
			if (!is_ValueChanged("M_PriceList_ID") && is_ValueChanged("DateOrdered")) {
				MPriceList priceList = MPriceList.get(getCtx(), getM_PriceList_ID(), get_TrxName());
				MPriceListVersion pricelistVersionOrder = priceList.getPriceListVersion((Timestamp) get_ValueOld("DateOrdered"), true);
				MPriceListVersion pricelistVersionActual = priceList.getPriceListVersion(getDateOrdered(), true);
				changePrices = (pricelistVersionOrder == null && pricelistVersionActual != null)
						...
						|| (pricelistVersionActual.getID() != pricelistVersionOrder.getID());
			}
			if (changePrices) {
				// Verificar los artículos que no se encuentran dentro de la
				// tarifa
				...
				for (MOrderLine orderLine : getLines()) {
					pp = new MProductPricing(orderLine.getM_Product_ID(), getC_BPartner_ID(), orderLine.getQtyEntered(), isSOTrx());
					...
					orderLine.setPrice(pp.getPriceList().subtract(
							pp.getPriceList().multiply(orderLine.getDiscount().divide(Env.ONEHUNDRED, 4, BigDecimal.ROUND_HALF_UP))));
					if (!orderLine.save()) {
	...
	/**
	 * Verifica si todos los artículos de las líneas se encuentran en la tarifa
	 * configurada
	 * 
	 * @param order
	 * @return
	...
	private CallResult isAllProductsInPriceList() {
		...
		String sql = "SELECT p.value, p.name "
				...
				+ "INNER JOIN m_product as p on p.m_product_id = ne.m_product_id " + "ORDER BY p.value";
		PreparedStatement ps = null;
		ResultSet rs = null;
		boolean allInPriceList = true;
		HTMLMsg msg = new HTMLMsg();
		HTMLMsg.HTMLList productList = msg.createList("products", "ul", Msg.getMsg(getCtx(), "ProductsNotInPriceList"));
		try {
			...
			while (rs.next()) {
				...
				msg.createAndAddListElement(rs.getString("value"), rs.getString("value") + " - " + rs.getString("name"), productList);
			}
	...
	protected boolean afterSave(boolean newRecord, boolean success) {
		...
		if (is_ValueChanged("Description") || is_ValueChanged("POReference")) {
			...
			if (DB.isPostgreSQL()) {
				sql = "UPDATE C_Invoice " + "SET Description=o.Description, POReference=o.POReference " + "FROM C_Order o "
						+ "WHERE C_Invoice.C_Order_ID=o.C_Order_ID " + "AND C_Invoice.DocStatus NOT IN ('RE','CL') " + "AND C_Invoice.C_Order_ID="
						+ getC_Order_ID();
			} else {
				sql = "UPDATE C_Invoice i" + " SET (Description,POReference)=" + "(SELECT Description,POReference "
		...
		if (is_ValueChanged("PaymentRule") || is_ValueChanged("C_PaymentTerm_ID") || is_ValueChanged("DateAcct") || is_ValueChanged("C_Payment_ID")
				|| is_ValueChanged("C_CashLine_ID")) {
			// begin DMA - Dataware - BugNo: 242
			String sql;

			if (DB.isPostgreSQL()) {
				sql = "UPDATE C_Invoice " + "SET PaymentRule=o.PaymentRule, C_PaymentTerm_ID=o.C_PaymentTerm_ID, DateAcct=o.DateAcct, "
						+ "    C_Payment_ID=o.C_Payment_ID, C_CashLine_ID=o.C_CashLine_ID " + "FROM C_Order o " + "WHERE C_Invoice.C_Order_ID=o.C_Order_ID "
						+ "AND C_Invoice.DocStatus NOT IN ('RE','CL') " + "AND C_Invoice.C_Order_ID=" + getC_Order_ID();
			} else {
				sql = "UPDATE C_Invoice i " + "SET (PaymentRule,C_PaymentTerm_ID,DateAcct,C_Payment_ID,C_CashLine_ID)="
						+ "(SELECT PaymentRule,C_PaymentTerm_ID,DateAcct,C_Payment_ID,C_CashLine_ID " + "FROM C_Order o WHERE i.C_Order_ID=o.C_Order_ID)"
	...
	private void afterSaveSync(String columnName) {
		...
		// if( is_ValueChanged( "DocStatus" ) || is_ValueChanged( "DatePromised"
		// ) || is_ValueChanged( "M_Warehouse_ID" )) {
		// if( (columnName.compareTo( "DocStatus" ) == 0) ||
		// (columnName.compareTo( "DatePromised" ) == 0) ||
		// (columnName.compareTo( "M_Warehouse_ID" ) == 0) ) {
		// // begin DMA - Dataware - BugNo: 242
		// String sql;
		//
		// if(DB.isPostgreSQL()) {
		...
		// int no = DB.executeUpdate( sql,get_TrxName());
		//
		// log.fine( columnName + " MPC_MRP set DocStatus --------di---" + no
		// +"   "+ sql);
		// }
		//
		// if( columnName.compareTo( "DocStatus" ) == 0 ) {
		...
		if (is_ValueChanged(columnName)) {
			...
			if (DB.isPostgreSQL()) {
				sql = "UPDATE C_OrderLine " + "SET " + columnName + "=o." + columnName + " " + "FROM C_Order o " + "WHERE C_OrderLine.C_Order_ID=o.C_Order_ID "
						+ "AND C_OrderLine.C_Order_ID=" + getC_Order_ID();
			} else {
				sql = "UPDATE C_OrderLine ol" + " SET " + columnName + " =" + "(SELECT " + columnName + " FROM C_Order o WHERE ol.C_Order_ID=o.C_Order_ID) "
	...
	public String prepareIt() {
		...
		/*
		 * TPV performance: Evitar invocacion (y doble) a getLines, manteniendo
		 * variable totalLines El seteo del numero de linea no tiene implicancia
		 * alguna, realizarlo via SQL Instanciar solo las lineas que sea
		 * necesario (el resto usar los IDs)
		 */
		/*
		 * int orderLineIDs[] = PO.getAllIDs("C_OrderLine", " C_Order_ID = " +
		 * getC_Order_ID(), get_TrxName()); int totalLines =
		 * orderLineIDs.length; int newLineNo = 10;
		 * 
		 * for (int lineID : orderLineIDs) { BigDecimal qtyOrdered =
		 * DB.getSQLValueBD(get_TrxName(),
		 * " SELECT COALESCE(qtyordered,0) FROM C_OrderLine WHERE C_OrderLine_ID = ?"
		 * , lineID); if (Env.ZERO.compareTo(qtyOrdered) == 0) { MOrderLine line
		 * = new MOrderLine(getCtx(), lineID, get_TrxName());
		 * if(!line.delete(true))
		 * log.severe("Could not delete 0-quantity order line. LineID=" +
		 * lineID); else totalLines--; } else { int cant =
		 * DB.executeUpdate(" UPDATE C_OrderLine SET line = " + newLineNo +
		 * " WHERE C_OrderLine_ID = " + lineID , get_TrxName()); if (cant==0)
		 * log.warning("Could not set the new line number. LineID="+lineID
		 * +", New Line=" + newLineNo); else newLineNo += 10; } }
		 * 
		 * // Lines
		 * 
		 * if( totalLines == 0 ) { m_processMsg = "@NoLines@";
		 * 
		 * return DocAction.STATUS_Invalid; }
		 */

		// Se borran las líneas del pedido que tienen cantidad igual a cero.
		// (borrar codigo comentado previo y esta misma linea una vez testeada
		// la mejora).
		// ADER: Performance para TPV Y Logica clásica. Las cosas se actualizan
		// SOLO si es encesario
		// Tambien, ante la falla de eliminación SE CANCELA; principalmente
		// porque PostGreSQL
		// tiene la cualidad de rollbackear automaticamente ante errores. Idem
		// para la reenumeración.
		// La eliminación de lineas con cantidad ordenada cero y la
		// reenumeración se hace por separado
		// Y SOLO si es necesario. Si la reenumeración es llevada a cabo, es
		// posible que
		// se tenga que reeler las lineas, si es que la lógica posterior lo
		// requiere; ver
		// m_needReloadForRenumber (si es true, las unico que puede ser distinto
		// en DB es el campo Line).
		...
		if (!ok) {
			m_processMsg = "Error eliminando lineas con cantidad ordenada cero"; // TODO:
																					// mejorar
																					// mensaje
			return DocAction.STATUS_Invalid;
		}
		...
		// en este punto m_lines tiene las lineas que realmente estan en la base
		// de datos
		// salvoq que se haya cometido un error previo. Lo unico que puede
		// diferir son los campos Line

		if (m_lines.length <= 0) { // m_lines es potencialmente actulizado or
									// deleteQtyOrdered
			m_processMsg = "@NoLines@";
			return DocAction.STATUS_Invalid;
		}

		// NOTA: luego de estas dos metodos, no es necesario forzar a reeleer
		// las lineas
		// los pasos que siguen, por ej explodeBOM, lo hacen explicitamente en
		// caso
		// de ser necesario.
		// FIN de Mejora I
		...
		DB.executeUpdate("UPDATE c_orderline SET pricelist = priceentered WHERE c_order_id = " + getID() + " AND pricelist = 0", get_TrxName());
			...
			if (getC_DocType_ID() != 0) {
				...
				if (MDocType.DOCSUBTYPESO_StandardOrder.equals(dtOld.getDocSubTypeSO()) // From
						...
						&& !MDocType.DOCSUBTYPESO_StandardOrder.equals(dt.getDocSubTypeSO())) // To
																								// !SO
				{
					// verificar que las lineas tengan el mismo warehouse que la
					// cabecera
					String sql = " select count(1) from ( " + "		select distinct m_warehouse_id " + " 		from c_orderline " + "		where c_order_id = "
							+ getC_Order_ID() + // warehouse de las lineas
							"		union select " + getM_Warehouse_ID() + // warehouse
																		// de la
																		// cabecera
							"	) as foo ";
		...
		String sql = " SELECT COUNT(*) " + "FROM C_OrderLine ol" + " INNER JOIN M_Product p ON (ol.M_Product_ID=p.M_Product_ID)"
				+ " INNER JOIN M_AttributeSet pas ON (p.M_AttributeSet_ID=pas.M_AttributeSet_ID) " + " WHERE pas.MandatoryType" + mandatoryType
		...
		/*
		 * int[] lines = PO.getAllIDs("C_OrderLine", "C_Order_ID = " +
		 * getC_Order_ID() , get_TrxName() ); if( !reserveStock( dt, lines )) {
		...
		boolean isOrderTransferable = dt.getDocTypeKey().equalsIgnoreCase(MDocType.DOCTYPE_Pedido_Transferible);
		// Para pedidos transferibles no se debe reservar stock
		if (!isOrderTransferable && !reserveStockII(dt)) {
			m_processMsg = "@CannotReserveStock@";
			return DocAction.STATUS_Invalid;
		}
		// NOTA: luego de este metodo, no es necesario releer las lineas ya que
		// calculeTaxTotal
		...
		// Validación de productos "Comprados" en las líneas del pedido si el
		// documento
		...
		if (dt.isDocType(MDocType.DOCTYPE_PurchaseOrder)) {
			...
			if (getGrandTotal().compareTo(bpartner.getMinimumPurchasedAmt()) < 0) {
				m_processMsg = Msg.getMsg(getCtx(), "GrandTotalNotSurpassMinimumPurchaseAmt", new Object[] { bpartner.getMinimumPurchasedAmt() });
				return DocAction.STATUS_Invalid;
			}
		}
	...
	private void explodeBOM() {
		String where = "AND IsActive='Y' AND EXISTS " + "(SELECT * FROM M_Product p WHERE C_OrderLine.M_Product_ID=p.M_Product_ID"
	 ...
	 * @return
	 */
	// TPV Performance: utilizar identificadores de lineas en lugar de los
	// objetos
	public boolean reserveStock(MDocType dt, int[] orderLineIDs) {
		...
		for (int i = 0; i < orderLineIDs.length; i++) {
			...
			if (header_M_Warehouse_ID != 0) // enforce WH
			{
				...
				DB.executeUpdate(" UPDATE C_OrderLine " + " SET M_Warehouse_ID = " + header_M_Warehouse_ID + ", AD_Org_ID = " + getAD_Org_ID()
						+ " WHERE C_OrderLine_ID = " + anOrderLineID, get_TrxName());
			}

			// Binding

			BigDecimal target = binding ? ol_QtyOrdered : Env.ZERO;
			BigDecimal difference = target.subtract(ol_QtyReserved.subtract(ol_QtyTransferred)).subtract(ol_QtyDelivered.add(ol_QtyTransferred));

			if (difference.compareTo(Env.ZERO) == 0 && ol_QtyTransferred.compareTo(BigDecimal.ZERO) <= 0) {
				continue;
			}

			log.fine("Line=" + ol_Line + " - Target=" + target + ",Difference=" + difference + " - Ordered=" + ol_QtyOrdered + ",Reserved=" + ol_QtyReserved
					+ ",Delivered=" + ol_QtyDelivered);
			...
			if ((product != null) && product.isStocked()) {
				...
				if (ol_M_AttributeSetInstance_ID != 0) { // Get existing
															// Location
				...
				if ((reserved.compareTo(Env.ZERO) != 0 || ordered.compareTo(Env.ZERO) != 0)
						&& !MStorage.add(getCtx(), ol_M_Warehouse_ID, M_Locator_ID, ol_M_Product_ID, ol_M_AttributeSetInstance_ID,
								ol_M_AttributeSetInstance_ID, Env.ZERO, reserved, ordered, get_TrxName())) {
					return false;
				}

				// update line

				DB.executeUpdate(" UPDATE C_OrderLine SET QtyReserved = " + ol_QtyReserved.subtract(ol_QtyTransferred).add(difference)
						+ " WHERE C_OrderLine_ID = " + anOrderLineID, get_TrxName());
			}
		} // reverse inventory
	...
	/***********************************************************************************
	 * Mejoras de performance y refactoring de lógica de documentos. Autor: Ader
	 * Javier - Cognitiva Consultores (Equipo de Desarrollo de Libertya)
	 * Principales metodos/secciones: -deleteQtyOrderedCeroIfNeeded(): elimina
	 * con a lo sumo un acceso a DB todas las lineas con cantidad oredenadas
	 * cero -renumberLinesIfNeeded(): reenumera las lineas (10,20,30...) con a
	 * lo sumo un acceso a DB -reserveStockII( MDocType dt ) : reserva stock y
	 * actualiza las lineas con a lo sumo 3 accesos a DB (sin contar con los
	 * accesos utilizados por MStorage en si mismo)
	 * 
	 * Todas estos metodos utilizan como punto de partida las MOrderLine's
	 * (pontencialmente cacheadas) via getLines(), y requieren condiciones
	 * minimas sobre las mismas. Ver comentarios. Tambien, como acceden
	 * directamente a la DB sin pasar por MOrderLine, las lineas cacheadas
	 * (m_lines) van a diferir con respecto a la DB (aunque en el caso de la
	 * reenumeración o eliminación de lineas, o seteo de OrgID y WhID, casi
	 * nunca se genera la discrepacia en escenarios normales) pero solo en pocos
	 * campos (QtyReserved, AD_Org_ID, M_Warehouse_ID, Line). Estos campos en
	 * general no son de mayor importancia en pasos posteriores de la logica de
	 * documentos asi que no deberia ser necesario, en muchos casos, releer las
	 * lineas desde DB. Ver métodos en particualar para detalles. -sección de
	 * cache de productos : getProductFromCache, initCacheProductFromLines, etc
	 */
	// TODO: siguientes flags no estan siendo seteados a false en caso de una
	// reelectura
	...
	/**
	 * si m_needReloadForQtyReservedUpdated = true, contiene la data usada la
	 * ultima ve que se invococo al metodo updateQtyReservedIfNeeded; con estos
	 * datos se puede inferir las discrepancia, si la logica lo requiere, entre
	 * las cacheadas actualmente y la DB.
	 */
	private DataReserveStock[] m_dataUsedInQtyReservedUpdated = null;

	/**
	 * cache de productos; puede ser seteada externamente en caso de escenarios
	 * multi-documentos
	 */
	 ...
	 * este ultimo caso, se actuliza la variable interna m_lines con la lista de
	 * aquellas que permanecen. Esto es despues de llamar a este metodo, no
	 * deberia ser necesario recargarlas. En caso de encontrar algun error se
	 * retorna false<br>
	 * NOTA: despues de esta eliminación es posible que la lineas requieran
	 * reenumeración
	 * 
	 * @return false si ocurre algun error (esto pasa si el DELETE no elimina
	 *         correctamente a las lineas)
	 */
	...
	/**
	 * Dadas las lineas obtenidas via getLines() las reenumera (campo Line) si
	 * es necesario. Es necesario si el conjunto de lineas no se pueden ordernar
	 * en una secuencia no creciente de la forma (10,20,30....), sin repetición.
	 * En caso de ser necesario lo reenumera generando UN solo acceso a DB;
	 * depues de esto los compos Line en la lineas (m_lines) van a diferir de lo
	 * dado en la DB, PERO SOLO este campo. En general, si la logica posterior
	 * no require que estos campos esten actualizados, no se deberia ser
	 * necesario la relectura de las lineas (y aún en este caso es posible
	 * inferir el order en base de datos sin accederla).<br>
	 * Como efecto colateral setea a false o true el campo
	 * m_needReloadForRenumber, si es que esta modificación se llevo a cabo.
	 * 
	 * @return false en caso de error al ejecutra (si es necesario) el UPDATE
	...
	private boolean renumberLinesIfNeeded() {
		...
		if (lines.length <= 0)
			return true;
		// 0-> 10, 1-> 20... Si este arreglo termina con todos true, es porque
		// NO
		...
		for (int i = 0; i < lines.length; i++) {
			...
			if ((numLine % 10) != 0) // tiene que ser multiplo de 10
				continue;
			int indexInNumLines = (numLine / 10) - 1; // 10 -> 0, 20 -> 1, 130
														// -> 12
			// tiene que estar dentro de los limites 0.. numLines.length-1; el
			// <0 no
		...
		// elimina los huecos y/o Line iguales y/o Lines no convencionales (17
		// por ej);
		// Los numeros de de linea resultante son SIEMPRE distintos
		// y dentro de la secuencia (10,20,30... ). Respeta el orden actual por
		// Line,
		// pero si hay dos C_OrderLine con igual Line queda con valor menor la
		// que tiene
		// menor C_OrderLine_ID (esto es, la que fue creada antes, toma valor
		// menor).
		// esta setencia acutaliza todas las lineas, aún cuando tomen el mismo
		// valor
		// de Line que tenian antes (posiblemente se pueda hacer de otra manera,
		// pero
		...
		// de procesamientos; tal vez se pueda mejorar usando la forma UPDATE
		// "UPDATE... FROM..."
		// NOTA: el select interno cuenta la cantidad de lineas que tiene numero
		// de linea menor
		// (ANTES del update; si este no fuera el caso, PostGreSQL tieen un bug
		// "conceptual")
		// o tiene el mismo número de Line pero menor C_OrderLine_ID. A esta
		// cantidad le suma 1
		// y lo multiplica por 10; de esta manera se genera SIEMPRE valores
		// postivos, distintos, múltiplos
		// de 10, comenzando en 10, y saltando de 10 en 10.
		String queryUpdate = "UPDATE C_OrderLine OL1 SET  " + " Line = " + "(SELECT 10 * (COUNT(1) + 1) FROM C_OrderLine OL2 " + "  WHERE "
				+ "    OL2.C_Order_ID = OL1.C_Order_ID AND " + "    (OL2.Line < OL1.Line " + "	 OR "
				+ "	 (OL2.Line = OL1.Line AND OL2.C_OrderLine_ID < OL1.C_OrderLine_ID) " + "	 ) " + ") " + "WHERE OL1.C_Order_ID = " + getC_Order_ID();

		int qtyUpdated = DB.executeUpdate(queryUpdate, get_TrxName());
		this.m_needReloadForRenumber = true;

		if (qtyUpdated != lines.length)
			return false; // por alguna razon no se actualizaron las Lines (o no
							// se actualizaron todas)

		return true;
	}

	/**
	 * Algoritmo de reservación de stock que es equivalente al original (salvo
	 * por bugs en este último), a partir de las lineas obtenidas via el metodo
	 * getLines(). Por si mismo genera solo 2 accesos a DB (potencialmente 3 si
	 * se requiere que las lineas y la cabecera tengan igual id Org e Id Wh que
	 * la cabecera y esto no se cumple); a esto hay que sumarles los N
	 * necesarios para actualizar M_Storage via MStorage.add(....), y otros N
	 * potenciales via MStorage.getM_Locator_ID si las lienas tienen seteado
	 * conjuntos de atributos).<br>
	 * El algoritmo anterior generaba N*3 y potencialmente N*4 accesos a DB por
	 * si mismo (a estos hay que agregarles los necesarios para actualizar el
	 * stock en si mismo). Esto es, sin contar los accesos de modificación de
	 * stock, se paso de un algortimo con cantidad de accesos lineal con
	 * respecto a la cantidad de lineas N, a uno con cantiad de accesos
	 * constante.<br>
	 * NOTA: este algoritmo requiere que las MOrderLines obtenidas via
	 * getLines() (y por lo tanto esten pontencialmente cacheadas) correspondan
	 * en número con las lineas en DB y en los campos usados por la clase
	 * DataReservedStock. Esto significa en particular que no se require relleer
	 * las lineas despues de haber invocado a metodos como
	 * deleteQtyOrderedCeroIfNeeded() (ya que este metodo deja en m_lines la
	 * lineas no eliminadas) o renumberLinesIfNeeded() (ya que este metodo solo
	 * modica los campos Lines de C_OrderLine, dato que no tiene importancia
	 * para la reservación de stock). <br>
	 * Finalmente, luego de invocar este método, las lineas obtenidas via
	 * getLines() pueden diferir con respecto a los datos en DB SOLO en los
	 * campos QtyOrdered, AD_Org_ID y M_Warehouese_ID (esto dos últimos menos
	 * problable). Estos son los únicos campos de las lineas que son modificados
	 * por este metodo accediendo directamente a la BD. De cualquier manera, las
	 * discrepacia, si es que existe, puede ser inferida posteriormente ( ver
	 * enforceWHandOrgIfNeeded() y updateQtyReservedIfNeeded(....))
	 * 
	 * @param dt
	 * @return false si ocurre algún error al intentar reservar stock o
	 *         actualizar las lineas para que reflejen tal reservación.
	 */
	public boolean reserveStockII(MDocType dt) {
		...
		// OR Closing Binding Quotation
				|| (MDocType.DOCSUBTYPESO_Quotation.equals(dt.getDocSubTypeSO()) && DOCACTION_Close.equals(getDocAction()))
				// OR isDropShip
				|| isDropShip()) {
			binding = false;
		}
		...
		MOrderLine[] lines = getLines();
		DataReserveStock[] data = getDataForReserveStock(lines, needEnforceWHandOrg, getAD_Org_ID(), getM_Warehouse_ID());
		...
		for (int i = 0; i < data.length; i++) {
			...
			BigDecimal target = binding ? item.QtyOrdered : Env.ZERO;
			BigDecimal difference = target.subtract(item.QtyReserved).subtract(item.QtyDelivered.add(item.QtyTransferred));

			if (difference.compareTo(Env.ZERO) == 0)
				continue;

			// saleo actulizacion de almacen si el producto asociao a la linea
			// lleva stock
			if (!item.IsStocked)
				continue;
			// EN ESTE PUNTO NO SE FUERZA Org y WH en Lineas, se hace mas tarde
			// Y SOLO si es requerido

			// Pedidos de Cliente (IsSoTrx) solo afecta M_Storage.qtyReserved
			// (se "reservan" para el pedido)
			...
			// Get Locator to reserve
			if (item.M_AttributeSetInstance_ID != 0) { // Get existing Location
				M_Locator_ID = MStorage.getM_Locator_ID(item.M_Warehouse_ID, // esto
																				// ademas
																				// corrige
																				// el
																				// bug
																				...
																				// codigo
																				// viejo
						item.M_Product_ID, item.M_AttributeSetInstance_ID, ordered, // FIXME?
																					// porque
																					// le
																					// pasa
																					// ordered?
			...
			// Get default Location TODO: nunca se mira el locator del
			// producto????
			if (M_Locator_ID == 0) {
			...
			// Update Storage
			ok = MStorage.add(getCtx(), item.M_Warehouse_ID, M_Locator_ID, item.M_Product_ID, item.M_AttributeSetInstance_ID, item.M_AttributeSetInstance_ID,
					Env.ZERO, // diffQtyOnHand - cantidad onHand queda igual
					reserved, // diffQtyReserved - cantidad reserveda: solo si
								// es IsSoTrx
					ordered, // diffQtyOrdered - cantidad ordenada: sol si no es
								// IsSoTrx
					get_TrxName());
			if (!ok) {
			...
			// NO update line ahora; solo se registra la diferencia que se tiene
			// que aplicar
	...
	/**
	 * Obtiene información (con a lo sumo UN SOLO acceso a DB) requerida por el
	 * algoritmo de reservación de stock , para un conjunto dado de lineas.
	 * 
	 * @param lines
	 *            MOrderLines a partir de la que se infiere la información
	 * @param needEnforceWHandOrg
	 *            si es false, los DataReserveStock's resultantes van a tener
	 *            los ids AD_Org_ID y M_Warehouse_ID tomados desde las lineas;
	 *            si true, tomados de los sig. parámetros
	 * @param AD_Org_ID
	 *            id de Org. a usar si needEnforceWHandOrg es true
	 * @param M_Warehouse_ID
	 *            id de WH. a usar si needEnforceWHandOrg es true
	 * @return retorna el arreglo (con un DataReservedStock por cada linea) o
	 *         null si hay un error
	 */
	private DataReserveStock[] getDataForReserveStock(MOrderLine[] lines, boolean needEnforceWHandOrg, int AD_Org_ID, int M_Warehouse_ID) {
		...
		for (int i = 0; i < lines.length; i++) {
			...
			MOrderLine ol = lines[i];
			if (!isForceReserveStock() && ol.getCheckoutPlace() != null && MProduct.CHECKOUTPLACE_PointOfSale.equals(ol.getCheckoutPlace())) {
				continue;
			}
			...
			// warehouse y org forzado, no se tiene en cuenta los ids de estos
			// campos
			// en las lineas si no que se toman de los parametros (de paso,
			// solucion
			...
			// en la lista de id de productos solo hay que saltear ids 0 y
			// repetidos
		...
		// OK, en este punto tenemos seteados todos los campos SALVO datos
		// relativos al producto
		// y su relación con el stock

		// caso simple (NO deberia pasar en general, salvo un pedido que tenga
		// lineas
		...
		String querySelectInfoProduct = "SELECT M_Product_ID,IsStocked,ProductType FROM M_Product WHERE M_Product_ID ";
		if (listIdProducts.size() == 1)
			querySelectInfoProduct = querySelectInfoProduct + " = " + listIdProducts.get(0);
		else
			querySelectInfoProduct = querySelectInfoProduct + " IN " + StringUtil.implodeForUnion(listIdProducts);
		...
		try {
			...
			while (rs.next()) {
				...
				// actualiza la info de producto en los DataReserveStock
				// correspondintes
				...
				for (DataReserveStock item : data) {
					...
					// se mantiene stock si el producto es un item (vs por ej,
					// un servicio)
					// y ademas esta marcado explicitamente como isStocked
					if (item.M_Product_IsStocked
							&& (MProduct.PRODUCTTYPE_Item.equals(item.M_Product_ProductType) || MProduct.PRODUCTTYPE_Assets.equals(item.M_Product_ProductType))) {
						item.IsStocked = true;
					}
				}
			}
	...
	/**
	 * Dadas las lineas obtenidas via getLines(), verifica si hay alguna con ID
	 * Org o ID WH distinta a la actual MOrder. Si hay al menos una, actualiza
	 * TODAS (por simplicidad) las lineas en DB seteandole el WhID y el OrgID de
	 * la MOrder actual (y en este caso setea el flag
	 * m_needReloadForEnforceWHandOrg a true), si no, no hace nada (y setea el
	 * flag m_needReloadForEnforceWHandOrg a false). Si la actualización se
	 * realiza, las lineas actualemente cacheadas (m_lines) SOLO puede diferir
	 * en el OrgID y el WhID con respecto a la DB, y en tal caso el valor en la
	 * DB puede inferirse simplemente mirando el OrgID y el WhID de la actual
	 * MOrder.<br>
	...
	private boolean enforceWHandOrgIfNeeded() {
		...
		// simplicando el algoritmo; si hay al menos una discrepacia se
		// actulizan
		// todas las lineas. El caso normal (el más habitual) es que no se
		// requiera
		// ningun update, ya que las lineas casi siempre son creadas con el Org
		// y
		...
		// se actualizan TODAS, esto retorna todas la lineas actulizadas (aun
		// cuando no cambien el valor)
		int qtyUpdated = DB.executeUpdate(" UPDATE C_OrderLine " + " SET M_Warehouse_ID = " + WhID + ", AD_Org_ID = " + OrgID + " WHERE C_Order_ID = "
	...
	/**
	 * A partir de los datos dados en un arreglo DataReserveStock (generados por
	 * el algoritmo de reservación de estock, en particular el campo
	 * DataReserveStock.differenceAppliedInStock y DataReserveStock.QtyReserved)
	 * actualiza las QtyReserved en base de datos (el valor seteado va a ser la
	 * suma de los dos campos anteiores). Esta acutalización solo se hace en
	 * caso de ser necesario (un DataReserveStock con diferencia distitta de
	 * cero), y en este caso se genera un solo acceso a DB para actulizar todas
	 * las lineas que lo requieran; tambien bajo esta ultima situción se setea
	 * el flag m_needReloadForQtyReservedUpdated a true y el parametro data se
	 * registra en m_dataUsedInQtyReservedUpdated por si logica posterior
	 * requiere saber que modicaciones se realizaron.
	 * 
	 * @param data
	...
	private boolean updateQtyReservedIfNeeded(DataReserveStock[] data) {
		...
		for (int i = 0; i < data.length; i++) {
			DataReserveStock item = data[i];
			if (item.differenceAppliedInStock.compareTo(BigDecimal.ZERO) == 0 && item.QtyTransferred.compareTo(BigDecimal.ZERO) == 0)
				continue;
			BigDecimal newQtyReserved = item.QtyReserved.add(item.differenceAppliedInStock).add(item.QtyTransferred);
	...
	private static class DataReserveStock {
		...
		int M_Warehouse_ID;
		int AD_Org_ID;
		// int Line; no es realmente requerida, y si m_needReloadForRenumber va
		// mostra una info incorrecta
		int M_Product_ID;
		...
		/**
		 * determina si lleva stock: calculado a partir de M_Product_IsStocket y
		...
		/**
		 * este campo es calculado por el algoritmo de reservación de stock y
		 * representa la "diferencia" aplicada. Si es cero, esta item no genero
		 * modificación de stock. Valor por defecto cero.
		 */
	...
	/**
	 * Metodo usado por logica de procesamientos multi-documento, para setear
	 * externamente la cache de productos (asi evitar que la logica de
	 * procesamiento la vuelva a generar).
	 * 
	 * @param cache
	 */
	...
	/**
	 * Obtiene de la cache de productos el asociado a M_Product_ID. Puede
	 * generar un accesos a DB indirectamente, si el producto no esta
	 * actulamente cacheado. <b> Retorna NULL si tal producto no se puede
	 * obtener; en general NO debe pasar esto, ya que signifca que tal producto
	 * no existe en la DB </b>
	 * 
	 * @param M_Product_ID
	 ...
	 * 
	 * @param lines
	 *            MOrderLine's a partir de la cual obtener los ids de los
	 *            productos a cargar
	...
	/**
	 * Retorna la cantidad de lineas accediendola desde base de datos. Un solo
	 * acceso a DB
	 */
	private int getQtyLinesFromDB() {
		...
		if (id <= 0)
			return 0;
		int qtyLines = DB.getSQLValue(get_TrxName(), "SELECT COUNT(1) FROM C_OrderLine WHERE C_Order_ID = " + id);
		return qtyLines;
	}
	...
	private boolean calculateTaxTotal() {
		...
		// FIXME Se agregó una condición para que si la tasa es de percepción
		// que
		...
		DB.executeUpdate("DELETE FROM C_OrderTax WHERE C_Order_ID=" + getC_Order_ID()
				+ " AND c_tax_id NOT IN (SELECT c_tax_id FROM c_tax WHERE ispercepcion = 'Y')", get_TrxName());
		m_taxes = null;
		...
		for (int i = 0; i < taxes.length; i++) {
			...
			if (tax.isSummary()) {
				...
				for (int j = 0; j < cTaxes.length; j++) {
					...
					// Si el tax hijo ya está insertado en el pedido, se suman
					// las cantidades
	...
	public void BreakLines(int M_Order_ID) {
		...
		try {
			...
			sql = new StringBuffer("SELECT c_orderline_id,qtyordered,c_bpartner_id,c_bpartner_location_id"
					...
					+ ",isdescription,processed,priceentered from c_orderline" + " where c_order_id=" + M_Order_ID);
			PreparedStatement pstmt = null;
			ResultSet rs;
			...
			while (rs.next()) {
				...
				if (quantity.compareTo(BigDecimal.ONE) == 1) {
					...
					for (int i = 1; i < quantity.intValue(); i++) {
						...
						// MOrderLine aux = new
						// MOrderLine(Env.getCtx(),rs.getInt(1),null);
						// aux_new= MOrder.co
					...
					int noLine = DB.executeUpdate(
							"UPDATE C_OrderLine set qtyordered=" + BigDecimal.ONE + ",qtyentered=" + BigDecimal.ONE + ",linenetamt="
									+ rs.getBigDecimal("priceactual") + " where c_orderline_id=" + rs.getInt(1), get_TrxName());
	...
	public String completeIt() {
		...
		initCacheProdFromLines(lines); // Ader: mejora de caches: TODO, falta
										// cache para MAttributeSet
		...
		for (int lineIndex = 0; lineIndex < lines.length; lineIndex++) {
			...
			// la mínima de compra
			if (!isSOTrx() && dt.getDocTypeKey().equals(MDocType.DOCTYPE_PurchaseOrder)) {
				MProductPO po = MProductPO.get(getCtx(), sLine.getM_Product_ID(), sLine.getC_BPartner_ID(), get_TrxName());
				if (po != null && sLine.getQtyEntered().compareTo(po.getOrder_Min()) < 0) {
					MProduct prod = MProduct.get(getCtx(), sLine.getM_Product_ID());
					m_processMsg = Msg.getMsg(getCtx(), "QtyEnteredLessThanOrderMinQty", new Object[] { prod.getValue(), prod.getName(), po.getOrder_Min(),
							sLine.getQtyEntered() });
					return DocAction.STATUS_InProgress;
				}
			}

			// La cantidad pedida no puede ser menor a la cantidad entregada +
			// la
			// cantidad transferida
			if (sLine.getQtyOrdered().compareTo(sLine.getQtyDelivered().add(sLine.getQtyTransferred())) < 0) {
			...
			// MProduct product = sLine.getProduct(); -> depende de la cache en
			// MProduct, que expira cada 5 min..
			MProduct product = getProductFromCache(sLine.getM_Product_ID());

			// puede ser NULL
			if (product == null)
				continue;
			// getM_AttributeSet_ID() con valor 0 represnta NULL,... NO tiene
			// sentido hacer un
			// new MAttributeSet con id 0, tal como esta el codigo acutalmente
			// eso es equivalente
			// a "crear" un nuevo MAttributeSet NO a obtener el que tiene id 0
			if (product.getM_AttributeSet_ID() <= 0)
				continue;
			if (product != null) {
				// TODO: hace cache MAttirbuteSet, inicalizarlar al principio y
				// usar getAttributeSetFromCache
				MAttributeSet atri = new MAttributeSet(Env.getCtx(), product.getM_AttributeSet_ID(), null);

				/*
				 * Modificar los m�todos Completeit de C_Order y M_InOut para
				 * que compruebe de nuevo que la mercanc�a tiene conjunto de
				 * atributos, si es necesario que los lleve, antes de completar.
				 */
				/*
				 * if (sLine.getM_AttributeSetInstance_ID() == 0 &&
				 * sLine.shouldSetAttrSetInstance()) { m_processMsg =
				 * "La l�nea " + sLine.getLine() +
				 * " no posee una instancia de conjunto de atributos; deber� especificarse el valor manualmente."
				 * ; return DocAction.STATUS_InProgress; }
				 */
				if (atri.isInstanceAttribute() && (sLine.getQtyEntered().compareTo(BigDecimal.ONE) == 1) && atri.isOrderInstanceAttribute()) {
					...
					m_lines = null; // por las dudas, aunque a continuación se
									// va a retornar
					// JOptionPane.showMessageDialog(
					// null,"Por favor, introduzca los números de serie y complete el pedido ",null,
					// JOptionPane.INFORMATION_MESSAGE );
		...
		if (MDocType.DOCSUBTYPESO_Proposal.equals(DocSubTypeSO) || MDocType.DOCSUBTYPESO_Quotation.equals(DocSubTypeSO)) {
			...
			if (MDocType.DOCSUBTYPESO_Quotation.equals(DocSubTypeSO)) {
				// int[] orderLines = PO.getAllIDs("C_OrderLine",
				// "C_Order_ID = " + getC_Order_ID(), get_TrxName() );
				// reserveStock( dt,orderLines);
				// Ader: Mejora II, de paso, se chequea por error
				// tal como lo hace prepareIt (PostGreSQL rollbackea
				// automaticamente...)
		...
		if (MDocType.DOCSUBTYPESO_POSOrder.equals(DocSubTypeSO) || MDocType.DOCSUBTYPESO_OnCreditOrder.equals(DocSubTypeSO)
				|| MDocType.DOCSUBTYPESO_PrepayOrder.equals(DocSubTypeSO)) {
		...
		/*
		 * Verificar, para los pedidos de clientes, que los productos tengan
		 * tildado el campo "Vendido" (checkbox en la ventana de artículos). La
		 * semántica de ese campo dice que es un artículo que se vende en la
		 * organización
		 */
		...
		if (docTarget.getDocTypeKey().equals(MDocType.DOCTYPE_StandarOrder)) {
			// Itero por las líneas del pedido verificando los artículos de
			// venta
			for (MOrderLine orderLine : lines) {
				// MProduct lineProduct =
				// MProduct.get(this.getCtx(),orderLine.getM_Product_ID());
		...
		if (isOrderTransferable) {
			int locatorID = MWarehouse.getDefaultLocatorID(getM_Warehouse_ID(), get_TrxName());
			for (MOrderLine orderLine : lines) {
				if (!Util.isEmpty(orderLine.getRef_OrderLine_ID(), true)) {
					MOrderLine refOrderLine = new MOrderLine(getCtx(), orderLine.getRef_OrderLine_ID(), get_TrxName());
					if (orderLine.getQtyOrdered().compareTo(refOrderLine.getPendingDeliveredQty()) > 0) {
						m_processMsg = "@QtyOrderedSurpassOrigLineQtyReserved@";
						return DocAction.STATUS_Invalid;
					}
					// Setear la cantidad transferida para que no se pueda
					// remitir
					...
					// Desreservar el stock del pedido original si el artículo
					// se stockea
					if (MProduct.isProductStocked(getCtx(), orderLine.getM_Product_ID())
							&& !MStorage.add(getCtx(), getM_Warehouse_ID(), locatorID, orderLine.getM_Product_ID(), 0, 0, BigDecimal.ZERO, orderLine
									.getQtyOrdered().negate(), BigDecimal.ZERO, get_TrxName())) {
						m_processMsg = "@CannotReserveStock@";
						return DocAction.STATUS_Invalid;
					}
				}
			...
			// automáticamente
			try {
				createTransferOrder(MDocType.getDocType(getCtx(), MDocType.DOCTYPE_Pedido_Transferido, get_TrxName()).getID());
			} catch (Exception e) {
		...
		// Si el Tipo de Documento Base es Pedido de Cliente y el Tipo de
		// Documento tiene seteado el
		// check box llamado “Imprimir al Completar”, entonces se ejecuta el
		// proceso "Nota de Pedido (Impresión)"
	...
	private MInOut createShipment(MDocType dt, Timestamp movementDate) {
		...
		for (int i = 0; i < oLines.length; i++) {
			...
			int M_Locator_ID = MStorage.getM_Locator_ID(oLine.getM_Warehouse_ID(), oLine.getM_Product_ID(), oLine.getM_AttributeSetInstance_ID(), MovementQty,
					get_TrxName());
		...
		/*
		 * String status = shipment.completeIt(); shipment.setDocStatus( status
		 * ); shipment.save( get_TrxName());
		 * 
		 * 
		 * if( !DOCSTATUS_Completed.equals( status )) { m_processMsg =
		 * "@M_InOut_ID@: " + shipment.getProcessMsg();
		 * 
		 * return null; }
		 */
	...
	private MOrder createCounterDoc() {
		...
		//
		/*
		 * if(counter){ MBPartner a = new
		 * MBPartner(Env.getCtx(),from.getC_BPartner_ID(),null);
		 * to.setAD_Org_ID(Integer.valueOf(a.getAD_OrgBP_ID()).intValue()); MOrg
		 * b = new
		 * MOrg(Env.getCtx(),Integer.valueOf(a.getAD_OrgBP_ID()).intValue
		 * (),null); MOrgInfo c = new MOrgInfo(b);
		 * to.setM_Warehouse_ID(c.getM_Warehouse_ID()); }
		 */
	...
	public boolean voidIt() {
		MDocType dt = MDocType.get(getCtx(), getC_DocType_ID());
		boolean isOrderTransferred = dt.getDocTypeKey().equalsIgnoreCase(MDocType.DOCTYPE_Pedido_Transferible);
		int locatorID = 0;
		...
		for (int i = 0; i < lines.length; i++) {
			...
			// transferidas del pedido original e incrementar la cantidad pedida
			// y
			// reservada del stock
			if (isOrderTransferred) {
				DB.executeUpdate(
				...
				// Reservar el stock del pedido original si el artículo se
				// stockea
				if (MProduct.isProductStocked(getCtx(), line.getM_Product_ID())
						&& !MStorage.add(getCtx(), getM_Warehouse_ID(), locatorID, line.getM_Product_ID(), 0, 0, BigDecimal.ZERO, old, BigDecimal.ZERO,
								get_TrxName())) {
					m_processMsg = "@CannotReserveStock@";
					return false;
				}
	...
	private boolean createReversals() {
		...
		if (isVoidInOuts()) {
			...
			for (int i = 0; i < shipments.length; i++) {
				...
				if (MInOut.DOCSTATUS_Closed.equals(ship.getDocStatus()) || MInOut.DOCSTATUS_Reversed.equals(ship.getDocStatus())
		...
		if (isVoidInvoices()) {
			...
			for (int i = 0; i < invoices.length; i++) {
				...
				if (MInvoice.DOCSTATUS_Closed.equals(invoice.getDocStatus()) || MInvoice.DOCSTATUS_Reversed.equals(invoice.getDocStatus())
		...
		if (!isOrderTransferred) {
			...
			for (int i = 0; i < lines.length; i++) {
				...
				if (old.compareTo(qtyDelivered) != 0) {
					...
					// Modificado por ConSerTi por el fallo de linea a 0 al
					// cerrar un pedido
	...
	public boolean reActivateIt() {
		...
		// ---------------------------------------------------------
		// // Actualización de los precios de las líneas según el precio actual
		// de tarifa.
		...
		// Si se reactiva un pedido transferible, en realidad debe sumar el
		// reservado
		boolean isOrderTransferred = dt.getDocTypeKey().equalsIgnoreCase(MDocType.DOCTYPE_Pedido_Transferible);
		...
		for (MOrderLine line : lines) {
			...
			if (isOrderTransferred) {
				updated = DB.executeUpdate("UPDATE c_orderline SET qtytransferred = qtytransferred-" + String.valueOf(line.getQtyOrdered())
						+ " WHERE c_orderline_id = " + line.getRef_OrderLine_ID(), get_TrxName());
				if (updated != 1) {
			...
			updated = DB.executeUpdate("UPDATE c_orderline SET qtyReserved = 0 WHERE c_orderline_id = " + line.getID(), get_TrxName());
			...
			if (MProduct.isProductStocked(getCtx(), line.getM_Product_ID())
					&& !MStorage.add(getCtx(), getM_Warehouse_ID(), locatorID, line.getM_Product_ID(), 0, 0, BigDecimal.ZERO, qtyReserved, qtyOrdered,
							get_TrxName())) {
				m_processMsg = "@CannotReserveStock@";
				return false;
			}
	 ...
	 *         alguna línea de este pedido y que no son están marcados como
	 *         "Comprados" (IsPurchased = 'N').
	 */
	private List<String> getNotPurchasedProducts() {
		List<String> products = new ArrayList<String>();
		String sql = " SELECT DISTINCT p.Name " + " FROM C_OrderLine ol " + " INNER JOIN M_Product p ON (ol.M_Product_ID = p.M_Product_ID) "
	...
	protected boolean updateAmounts() {
		...
		dataSql.append("SELECT ")
				...
				.append("(SELECT COALESCE(SUM(LineNetAmt),0) ").append("FROM C_OrderLine ol ").append("WHERE ol.C_Order_ID = o.C_Order_ID) AS TotalLines, ")
				// Suma de los impuestos de este pedido
				.append("(SELECT COALESCE(SUM(TaxAmt),0) ").append("FROM C_OrderTax ot ").append("WHERE ot.C_Order_ID = o.C_Order_ID) AS TotalTaxAmt ")
				.append("FROM C_Order o ").append("WHERE o.C_Order_ID = ?");
		...
		try {
			...
			pstmt.setInt(1, getC_Order_ID());
			rs = pstmt.executeQuery();
			// Obtiene los importes necesarios para actualizar el total del
			// pedido.
			if (rs.next()) {
	...
	public boolean isExpiredProposal(Date date) {
		return MDocType.DOCSUBTYPESO_Proposal.equals(getDocSubTypeSO()) && getValidTo() != null && getValidTo().compareTo(date) < 0
				&& !TimeUtil.isSameDay(getValidTo(), new Timestamp(date.getTime()));
	}

	/**
	 * @param voidInOuts
	 *            the voidInOuts to set
	 */
	...
	public BigDecimal getTotalLinesNetWithoutDocumentDiscount() {
		...
		for (MOrderLine orderLine : getLines()) {
			// Total de líneas sin impuestos
			total = total.add(orderLine.getTotalPriceEnteredNet()).subtract(orderLine.getTotalDocumentDiscountUnityAmtNet());
		}
		return total;
	}
	...
	public BigDecimal getPercepcionesTotalAmt() {
		String sql = "select sum(taxamt) " + "from c_ordertax as ot " + "inner join c_tax as t on t.c_tax_id = ot.c_tax_id "
	...
	public List<MOrderTax> getAppliedPercepciones() {
		String sql = "select ot.* " + "from c_ordertax as ot " + "inner join c_tax as t on t.c_tax_id = ot.c_tax_id "
				+ "where ot.c_order_id = ? AND t.ispercepcion = 'Y'";
		PreparedStatement ps = null;
		ResultSet rs = null;
	...
	public void changeOrderWarehouse(Integer M_Warehouse_ID) throws Exception {
		if (getDocStatus().compareTo(DOCSTATUS_Drafted) == 0) {
			...
			if (getDocStatus().compareTo(DOCSTATUS_Completed) == 0) {
				...
				setM_Warehouse_ID(M_Warehouse_ID);
				// Guardar la cabecera de pedido antes de modificar las líneas,
				// dado que las líneas recuperan la cabecera
				// a fin de determinar si es necesario modificar el warehouse a
				// partir de la información de la cabecera
				if (!save()) {
	...
	public BigDecimal getTotalDiscountAmtFromLines(String discountColumnName) {
		discountColumnName = !Util.isEmpty(discountColumnName, true) ? discountColumnName : "documentdiscountamt+linediscountamt+linebonusamt";
		String sql = "select sum(" + discountColumnName + ") from c_orderline where c_order_id = ?";
		BigDecimal amt = DB.getSQLValueBD(get_TrxName(), sql, getID());
		return Util.isEmpty(amt, false) ? BigDecimal.ZERO : amt;
	}
