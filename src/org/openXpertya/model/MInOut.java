...
//            MOrderLine ol = null;
            ...
            if( sLine.getC_OrderLine_ID() != 0 ) {
                ...
                if (isSOTrx()) {
            		...
            		/*
            		 * GENEOS - Comentado por Modificacion para que se permita recibir mas que lo ordenado en el pedido
            		 */
            		/*
...
//                }
                ...
                if( mtrx == null ) {
					...
					if (!Util.isEmpty(sLine.getC_OrderLine_ID(), true)
						...
						if (!MStorage.add(getCtx(), ol_warehouseID,
								...
								reservationAttributeSetInstance_ID, BigDecimal.ZERO,
								QtySO, tmpQtyPO.negate(), get_TrxName())) {
                ...
                if( ol_ok ) {    // other in VMatch.createMatchRecord
                	if(isSOTrx()){
                		...
                		//Pablo - Geneos: Estas variables no se porque se setean, no son utilizadas si !isSOTrx()
                		if(MovementType.endsWith("+")){
                			ol_qtyReserved = ol_qtyReserved.subtract( sLine.getMovementQty());
                		}
                		else{
